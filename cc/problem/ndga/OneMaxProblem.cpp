...
void OneMaxProblem::evaluate(Individual &individual) {
    ...
    for (int k = 0; k < chromosome.size(); k++) {
        ...
        zeros += 1 - chromosome.at(k);
    }

    dynamic_cast<RawFitness &>(individual.getRelevance().getFitness()).fitness(ones);
    dynamic_cast<RawFitness &>(individual.getRelevance().getFitness()).standarizedFitness(zeros);
}